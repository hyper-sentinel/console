...
} from '/var/task/Antigravity/webdev/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Antigravity/webdev/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Antigravity/webdev')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Antigravity/webdev',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Antigravity/webdev/.netlify/dist/run/handlers/server.js')
